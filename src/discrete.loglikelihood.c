...
int i = 0, k = 0;
int *xx = INTEGER(x), llx = NLEVELS(x),  num = length(x);
double *n = NULL, *ww = REAL(w), res = 0, total = 0;

  /* initialize the contingency table. */
  n = Calloc1D(llx, sizeof(double));
...
int llx = NLEVELS(x), lly = NLEVELS(y), num = length(x);
int *xx = INTEGER(x), *yy = INTEGER(y);
double res = 0, *ww = REAL(w), **n = NULL, *nj = NULL;
