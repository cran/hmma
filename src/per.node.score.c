...
SEXP cur, iss, prior, beta, exp, phi, l, weights;
  ...
  switch(s) {
    ...
    /* Weighted conditional Linear Gaussian log-likelihood score. */
    case LOGLIK_CGW:
      weights = getListElement(extra_args, "weights");
      ...
      for (i = 0; i < ntargets; i++) {
        ...
        SET_STRING_ELT(cur, 0, STRING_ELT(targets, i));
        DEBUG_BEFORE();
        res[i] = loglik_cgnode_w(cur, network, data, weights, NULL, debuglevel);
    ...
    case BIC_W:
      ...
      for (i = 0; i < ntargets; i++) {
        ...
        SET_STRING_ELT(cur, 0, STRING_ELT(targets, i));
        DEBUG_BEFORE();
        res[i] = loglik_dnode_w(cur, network, data, weights, &nparams, debuglevel);
        res[i] -= (*k) * nparams;
    ...
    case BIC_CGW:
      ...
      for (i = 0; i < ntargets; i++) {
        ...
        SET_STRING_ELT(cur, 0, STRING_ELT(targets, i));
        DEBUG_BEFORE();
        res[i] = loglik_cgnode_w(cur, network, data, weights, &nparams, debuglevel);
        res[i] -= (*k) * nparams;
