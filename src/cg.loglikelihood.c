...
double ccgloglik_w(SEXP x, SEXP w, SEXP parents, int *type, int np, int ndp,
    double *nparams) {
...
SEXP temp;
  ...
  res = c_fast_ccgloglik_w(REAL(x), REAL(w), gp, ngp, nobs, config, nconfig);
...
double res = 0, *fitted = NULL, *sd = NULL;
  ...
  if (!config) {
    ...
    /* if the model is not singular compute the log-likelihood. */
    for (j = 0; j < nobs; j++)
      res += log(ww[i]*dnorm(xx[j], fitted[j], sd[config[j] - 1], FALSE));
...
SEXP nodes, node_t, parents, data_t, parent_vars, config;
  ...
  if (nparents == 0) {
    ...
    /* no parents, reuse the marginal likelihoods. */
    if (TYPEOF(data_t) == INTSXP)
      loglik = dlik_w(data_t, w, nparams);
    else
      loglik = glik_w(data_t, w, nparams);
    ...
    if (TYPEOF(data_t) == INTSXP) {
     ...
     if (dparents != nparents) {
       ...
       PROTECT(config = c_configurations(parent_vars, TRUE, TRUE));
       loglik = cdlik_w(data_t, w, config, nparams);
       UNPROTECT(1);
      ...
      if (dparents == 0)
        loglik = cglik_w(data_t, data, w, parents, nparams);
      else
        loglik = ccgloglik_w(data_t, w, parent_vars, type, nparents, dparents,
                   nparams);
